...
export function findContent(type: "lesson" | "story", id: string): Content | undefined {
  return type === "lesson" ? lessonById.get(id) : storyById.get(id);
}
...
const bodyLoaders: Record<Content["type"], Record<string, () => Promise<string>>> = {
...
export async function loadContentBody(item: Content): Promise<string> {
  const cacheKey = `${item.type}:${item.id}`;
  const cached = bodyCache.get(cacheKey);
  if (cached !== undefined) return cached;

  const directory = item.type === "lesson" ? "lessons" : "stories";

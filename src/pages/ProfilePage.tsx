import { ArrowRight, BookOpenText, Books, Gear, Stack, UserCircle } from "@phosphor-icons/react";
...
export function ProfilePage() {
  ...
  return (
    <DarkShell className="profile-shell">
      ...
      <section className="profile-next-section"><p className="eyebrow">ПРОДОЛЖИТЬ ОБУЧЕНИЕ</p>{nextLesson ? <Link to={`/lessons/${nextLesson.id}`} className="next-card"><div><span>Урок {String(nextLesson.order).padStart(2, "0")} · {nextLesson.level ?? "A1"}</span><strong>{nextLesson.title}</strong><small>{nextLesson.wordCount ?? "—"} слов</small></div><ArrowRight size={22} /></Link> : <p className="empty-copy">Все уроки пройдены.</p>}</section>
